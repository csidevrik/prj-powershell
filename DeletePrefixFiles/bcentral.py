...
class FacturaBcentral:
    def __init__(self, f_fechaE, f_numberF, f_descripcion, f_valueSinImp):
        self.f_fechaE = f_fechaE
        self.f_numberF= f_numberF
        self.f_descripcion = f_descripcion
        self.f_valueSinImp = f_valueSinImp
...
def process_all_xml_facs(directory_path):
    registros = []
    ...
    for filename in os.listdir(directory_path):
        if filename.endswith(".xml"):
            ...
            # Extrae los datos del archivo XML
            registro = get_register_xml_factura(xml_file_path)

            # Agrega el registro a la lista
            registros.append({
                'fecha': registro.f_fechaE,
                'factura': registro.f_numberF,
                'descrip': registro.f_descripcion,
                'valor': registro.f_valueSinImp
            })
...
def get_register_xml_factura(xml_file_path):
    try:
        with open(xml_file_path, 'r', encoding='utf-8') as file:
            xml_content = file.read()

        # Buscar el contenido de la factura dentro del XML completo
        start_tag = "<factura"
        end_tag = "</factura>"
        start_index = xml_content.find(start_tag)
        end_index = xml_content.find(end_tag) + len(end_tag)

        if start_index == -1 or end_index == -1:
            print(f"No se encontró la estructura de factura en el archivo: {xml_file_path}")
            return None

        # Extraer solo el XML de la factura
        factura_xml = xml_content[start_index:end_index]
        ...
        fac_numbe = f"{estab}{pto_em}{secue}"
        fac_value = root.find(".//totalSinImpuestos").text
        fac_fechaE = root.find(".//fechaEmision").text
        fac_descrip = root.find(".//descripcion").text
        fac_number = f"FAC{fac_numbe}"

        # Log para debug
        print(f"Fecha: {fac_fechaE}")
        print(f"Número: {fac_numbe}")
        print(f"Descripción: {fac_descrip}")
        print(f"Valor: {fac_value}")
        print("///////////////////////////////////////////////")

        return FacturaBcentral(
            f_fechaE=fac_fechaE,
            f_numberF=fac_numbe,
            f_descripcion=fac_descrip,
            f_valueSinImp=fac_value
        )
    except Exception as e:
        print(f"Error procesando el archivo {xml_file_path}: {str(e)}")
        return None
...
def rename_facturas_bcentral(folder_path):
    # Ruta de la carpeta "corregir"
    ruta_corregir = os.path.join(folder_path, "corregir")
    print(ruta_corregir)
    ...
    for xml_file in xml_files:
        # Obtener el nuevo nombre del archivo XML
        registrofactura = get_register_xml_factura(os.path.join(folder_path, xml_file))
        new_name = registrofactura.f_numberF
    ...
    def main(page: ft.Page):
        ...
        pb = ft.PopupMenuButton(
            items=[
                ...
                ft.PopupMenuItem(
                    icon=ft.Icons.TEXT_FORMAT_ROUNDED,  text="Remove prefix Factura_", on_click=lambda e: remove_prefix_files_pdf(directory_path.value,"Factura_")
                    ),
                ft.PopupMenuItem(
                    icon=ft.Icons.TEXT_FORMAT_ROUNDED,  text="Rename files using the xml", on_click=lambda e: rename_facturas_bcentral(directory_path.value)
                    ),
                ft.PopupMenuItem(
                    icon=ft.Icons.TEXT_FORMAT_ROUNDED,  text="Process all xml files facturas for json", on_click=lambda e: process_all_xml_facs(directory_path.value)
                    ),
                ft.PopupMenuItem(
